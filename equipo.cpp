...
void Equipo::jugador(int nro_jugador)
{
	...
	if (!soy_el_mas_cercano && this->strat == SHORTEST)
		return;
	while (!this->belcebu->termino_juego())
	{
		...
		case (RR):
		{
			...
			{
				belcebu->termino_equipo_rr(this->equipo);
			}
			break;
		}
		case (SHORTEST):
		{
			...
			if (belcebu->ronda_actual() == 0 && this->equipo == ROJO)
			{
				...
				for (size_t i = 0; i < cant_jugadores - 1; i++)
					sem_wait(&belcebu->turno_rojo);
			}
			   ...
			   estrategia strat, int cant_jugadores, int quantum, vector<coordenadas> posiciones)
{
	...
	for (int i = 0; i < cant_jugadores; i++)
	{
		sem_t semaphore;
		if (equipo == ROJO)
			belcebu->mutexes_rr_rojos.emplace_back(semaphore);
		else
			belcebu->mutexes_rr_azules.emplace_back(semaphore);
		if (i == 0)
		{
			if (equipo == ROJO)
				sem_init(&(belcebu->mutexes_rr_rojos[i]), 0, 1);
			else
				sem_init(&(belcebu->mutexes_rr_azules[i]), 0, 0);
		}
		else
		{
			if (equipo == ROJO)
				sem_init(&(belcebu->mutexes_rr_rojos[i]), 0, 0);
			else
				sem_init(&(belcebu->mutexes_rr_azules[i]), 0, 0);
		}
	}
...
void Equipo::comenzar()
{
	...
	if (!NAIVE_SEARCH)
		clock_gettime(CLOCK_REALTIME, &tiempo_antes_threaded);
